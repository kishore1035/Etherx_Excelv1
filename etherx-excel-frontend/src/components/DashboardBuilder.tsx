...
export function DashboardBuilder({ open, onClose }: DashboardBuilderProps) {
  ...
  const renderWidget = (widget: Widget) => {
    return (
      <div
        key={widget.id}
        className={`absolute bg-card border rounded-lg shadow-sm cursor-move dashboard-widget ${
            ...
            <div className="w-full h-full flex items-end justify-around gap-2 px-4">
              {[60, 85, 45, 95].map((height, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-2">
                  <div
                    className={`w-full bg-primary rounded-t dashboard-bar-height`}
              ...
              <>
                <div className="p-4 border-t border-border">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label htmlFor="widget-title" className="text-xs text-muted-foreground">Title</label>
                      <input
                        id="widget-title"
                        type="text"
                        ...
                        title="Widget Title"
                        onChange={e => {
                          const updated = widgets.map(w => w.id === selectedWidget ? { ...w, title: e.target.value } : w);
                          setWidgets(updated);
                        }}
                      />
                    </div>
                    ...
                    <div>
                      <label htmlFor="data-source" className="text-xs text-muted-foreground">Data Source</label>
                      <select id="data-source" className="w-full mt-1 px-3 py-2 rounded border border-border bg-background text-sm" title="Data Source">
                    ...
                    <div>
                      ...
                      <select id="color-scheme" className="w-full mt-1 px-3 py-2 rounded border border-border bg-background text-sm" title="Color Scheme">
                        <option>Blue</option>
                        <option>Green</option>
